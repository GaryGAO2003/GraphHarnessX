    ...
    async def evaluate_with_trace_judge(
        ...
        *,
        ...
        def _window(text: str) -> str:
            """Keep both ends of an oversized turn, never just the head.

            The grading procedure below asks for the sentence where the agent
            COMMITS, and a commit lands at the end of the turn that makes it.
            Head-only truncation therefore deletes exactly the thing being looked
            for: measured on M14's R0, two of thirty-six failing tasks had a
            correct FINAL ANSWER past the cut — one at char 27,249 of 27,275 —
            so the judge saw no commit, wrote QUOTE: NONE, and step 3 makes that
            a mandatory FAIL. Roughly two tasks a round, always in the same
            direction.

            The head is kept as well because the opening states what the turn set
            out to do, which the judge needs to tell a commit from a restatement.
            """
            if len(text) <= max_chars_per_msg:
                return text
            head = max_chars_per_msg // 3
            tail = max_chars_per_msg - head
            return f"{text[:head]}\n\n[... {len(text) - max_chars_per_msg} chars elided ...]\n\n{text[-tail:]}"

        # Latest first so the judge reads the commit-point first.
        recent_rendered = "\n\n---\n\n".join(
            _window(m.content or "") for m in reversed(recent)
        )
