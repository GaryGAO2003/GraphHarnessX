...
_GATE = ghx._GATE
_GUIDANCE = ghx._GUIDANCE
_ALL_FLAGS = (_UNFOLD, _IDENTITY, _EVIDENCE, _GUIDANCE, _GATE, "HARNESSX_GHX_RUNTIME")
...
def test_ghx_level_default_and_choices():
    ...
    assert parser.parse_args(["--ghx-level", "6"]).ghx_level == 6  # the campaign profile
    with pytest.raises(SystemExit):
        parser.parse_args(["--ghx-level", "7"])  # off the ladder


def test_level_6_is_level_5_plus_every_proven_seam_minus_strategy_pop():
    """The profile exists so two arms can be byte-certain they ran the same
    set. STRATEGY_POP is frozen out (self-agreement 0/6, no consumer), so its
    absence is part of the contract, not an oversight."""
    six = set(ghx.LEVEL_FLAGS[6])
    assert set(ghx.LEVEL_FLAGS[5]) < six
    assert "HARNESSX_GHX_STRATEGY_POP" not in six
    assert {"HARNESSX_GHX_LAYER_A", "HARNESSX_GHX_ATTRIBUTION",
            "HARNESSX_GHX_SINGLESHOT_DIGESTER", "HARNESSX_GHX_ANCHOR_REPAIR"} < six
    applied = ghx.apply_level_flags(6, environ={})
    assert set(applied) == six
...
def test_history_filter_drops_tasks_this_lineage_has_already_solved(tmp_path):
    """At k=1 a single draw decides, and this bed's rollouts disagree on ~21% of
    tasks — so a raw failing set carries ~23% lottery losers into the cones, the lift
    numerator and every Digester. Salting the numerator with non-failures pulls every
    node's lift toward 1.0, which is the separation GHX's claim rests on.

    A task the lineage has already solved is not a failure of it, whatever this
    round's one draw said.
    """
    import json

    from recipe.gaia_evolver.run_meta_aegis_ghx import _ever_passed

    d = tmp_path / "data"
    d.mkdir()
    rows = [
        {"round": 0, "task_id": "solved-earlier", "passed": True, "passed_flags": [True]},
        {"round": 0, "task_id": "always-fails", "passed": False, "passed_flags": [False]},
        {"round": 1, "task_id": "solved-later", "passed": True, "passed_flags": [True]},
    ]
    (d / "task_history.jsonl").write_text(
        "\n".join(json.dumps(r) for r in rows) + "\n", encoding="utf-8"
    )

    # Deriving the failing set for round 1 may only look at rounds < 1.
    assert _ever_passed(tmp_path, before_round=1) == {"solved-earlier"}
    # For round 2 both earlier passes count.
    assert _ever_passed(tmp_path, before_round=2) == {"solved-earlier", "solved-later"}


def test_history_filter_is_empty_without_history(tmp_path):
    """Round 1 has no history and must not invent one. It is also the round that
    matters least — it ships nothing."""
    from recipe.gaia_evolver.run_meta_aegis_ghx import _ever_passed

    assert _ever_passed(tmp_path, before_round=1) == set()
    (tmp_path / "data").mkdir()
    (tmp_path / "data" / "task_history.jsonl").write_text("not json\n", encoding="utf-8")
    assert _ever_passed(tmp_path, before_round=1) == set()


def test_resume_preflight_unpoisons_merged_after_rollback(tmp_path, monkeypatch):
    """M24 fix #3: a resume whose last round rolled back must NOT reinstate the
    reverted ships — merged.yaml is replaced with the validated lineage's
    config and the poisoned merge is kept as a backup."""
    import argparse
    import json

    import recipe.gaia_evolver.run_meta_aegis_ghx as launcher

    run = tmp_path / "runs" / "arm"
    (run / "R0").mkdir(parents=True)
    (run / "R1" / "applied").mkdir(parents=True)
    (run / "R0" / "config.yaml").write_text("seed: true\n", encoding="utf-8")
    (run / "R1" / "applied" / "merged.yaml").write_text("seed: true\nship: poisoned\n", encoding="utf-8")
    (run / "audit.jsonl").write_text(
        json.dumps({"round": 1, "kind": "rollback", "payload": {"rolled_back_cids": ["C-R1-01"]}}) + "\n",
        encoding="utf-8",
    )
    monkeypatch.setattr(launcher._pilot, "RUNS_DIR", tmp_path / "runs")

    args = argparse.Namespace(start_round=2, run_tag="arm")
    launcher._fix_merged_after_rollback(args)

    merged = (run / "R1" / "applied" / "merged.yaml").read_text(encoding="utf-8")
    assert merged == "seed: true\n"  # restored to R0's validated config
    backup = run / "R1" / "applied" / "merged.pre_rollback_resume.yaml"
    assert backup.exists() and "poisoned" in backup.read_text(encoding="utf-8")

    # idempotent-ish second call: R1 still rolled back, merged now == R0 config,
    # backup simply rewritten — and a rollback-free resume is a no-op
    launcher._fix_merged_after_rollback(args)
    (run / "audit.jsonl").write_text("", encoding="utf-8")
    (run / "R1" / "applied" / "merged.yaml").write_text("anything\n", encoding="utf-8")
    launcher._fix_merged_after_rollback(args)
    assert (run / "R1" / "applied" / "merged.yaml").read_text(encoding="utf-8") == "anything\n"
