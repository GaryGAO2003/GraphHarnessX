...
class TestApplyEdits:
    ...
    # ── mutate_params (M23) — kwargs-only change on an existing node ─────────

    def test_mutate_params_changes_kwargs(self):
        g = make_minimal_graph()
        edit = GraphEdit(
            edit_type=GraphEditType.MUTATE_PARAMS,
            target_node_id="proc:a",
            node_changes={"threshold": 5},
        )
        g2 = apply_edits(g, [edit])
        assert g2.nodes["proc:a"].metadata["threshold"] == 5
        # original untouched (immutable style)
        assert "threshold" not in g.nodes["proc:a"].metadata

    def test_mutate_params_updates_ctor_kwargs_mirror(self):
        g = make_minimal_graph()
        g.nodes["proc:a"].metadata["_ctor_kwargs_"] = {"threshold": 1}
        edit = GraphEdit(
            edit_type=GraphEditType.MUTATE_PARAMS,
            target_node_id="proc:a",
            node_changes={"threshold": 7},
        )
        g2 = apply_edits(g, [edit])
        assert g2.nodes["proc:a"].metadata["_ctor_kwargs_"]["threshold"] == 7
        assert g2.nodes["proc:a"].metadata["threshold"] == 7

    def test_mutate_params_refuses_underscore_metadata(self):
        g = make_minimal_graph()
        edit = GraphEdit(
            edit_type=GraphEditType.MUTATE_PARAMS,
            target_node_id="proc:a",
        ...
        with pytest.raises(GraphEditError, match="underscore"):
            apply_edits(g, [edit])

    def test_mutate_params_unknown_target(self):
        g = make_minimal_graph()
        edit = GraphEdit(
            edit_type=GraphEditType.MUTATE_PARAMS,
            target_node_id="proc:nope",
            node_changes={"x": 1},
        )
