...
async def test_wired_round_pointer_only_for_failed_tasks_with_evidence(tmp_path):
    ...
    # FAILED task: the pointer section (listing only files that exist) is appended,
    # followed by the M23 digester enrichment — the cone-first protocol naming the
    # cone's own causal steps, and the full-id anchor teaching (anti-'...''-elision).
    assert _expected_section([str(cone_alpha), str(facts)]) in alpha_prompt
    assert "Cone-first protocol" in alpha_prompt
    assert "`trajectories/alpha_r0.jsonl#step_3`" in alpha_prompt
    assert "never abbreviate" in alpha_prompt
