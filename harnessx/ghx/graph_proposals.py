...
class _CandidateState:
    ...
    # tools-bucket edits (insert_tool / remove_tool): applied to the candidate
    # config's tool_registry.custom at render time; never touch the snapshot.
    tool_edits: list = field(default_factory=list)
...
# ── tools-bucket edits (config-section ops, not processor-graph edits) ───────
#
# The tools bucket was the one direction the graph-edit surface could not
# express (M24: the Critic flagged `tools` from R2 on, IV-11 taxed every
# detour, and the Evolver's own manifesto called the bucket "structurally
# inexpressible"). These two edit types close that gap at the level AEGIS
# compose actually carries (`_apply_tools` merges the tool_registry.custom
# add/drop diff): they mutate the candidate config's ``tool_registry.custom``
# list — NOT the processor snapshot — so genotype hash and S0–S4 preflight are
# untouched, and ``derive_landing_buckets`` derives ``tools`` from the diff.
# A custom tool whose name collides with a builtin OVERRIDES it at load time
# (core loader falls back to ``register(replace=True)`` with a warning), which
# is exactly the shape the Bash/cmd.exe root-cause fix needs.

_TOOL_EDIT_TYPES = ("insert_tool", "remove_tool")


def _resolve_tool_entry(entry: str) -> str:
    """Import-preflight a ``tool_registry.custom`` entry; return the tool name.

    Same resolution the core loader uses (``file://`` URI or module path), so
    an edit that passes here loads at run time. Raises
    :class:`_EditConversionError` on any failure — the edit is rejected while
    the model can still fix it (the same write-what-we-would-refuse-to-read
    asymmetry ``_write_config_verified`` closes for processors).
    """
    import importlib
    ...
    try:
        if entry.startswith("file://"):
            from ..core.harness import _parse_file_tool_target

            path_part, sym = _parse_file_tool_target(entry)
            if not Path(path_part).exists():
                raise FileNotFoundError(f"no such file: {path_part}")
            spec = _iu.spec_from_file_location(f"_ghx_tool_preflight_{_uuid.uuid4().hex}", path_part)
            ...
            mod_path, sym = entry.rsplit(".", 1)
            fn = getattr(importlib.import_module(mod_path), sym)
    except Exception as exc:  # noqa: BLE001 — surfaced as a structured edit rejection
        raise _EditConversionError(
            f"insert_tool entry {entry!r} failed import preflight: {type(exc).__name__}: {exc}"
        ) from None
    name = getattr(fn, "name", None) or getattr(fn, "__name__", None)
    if not name:
        raise _EditConversionError(f"insert_tool entry {entry!r} resolves to an object with no tool name")
    return str(name)


def _tool_entry_stem(entry: str) -> str:
    """Best-effort short name for an entry that no longer imports (remove path)."""
    tail = entry.rsplit("::", 1)[-1].rsplit(".", 1)[-1].rsplit("/", 1)[-1]
    return tail or entry
...
_EDIT_DESC = (
    ...
    "(snapshot and files untouched). Seven edit_type values: insert_node, remove_node, "
    "replace_same_group, change_dependency, swap_subgraph, mutate_inactive, "
    "mutate_params. To change an EXISTING processor's constructor kwargs (active or "
    "not), use mutate_params with node_changes={kwarg: new_value} -- it refuses "
    "underscore metadata outright, so the metadata trap cannot fire through it, and "
    "its diff derives bucket=config. replace_same_group remains for swapping the "
    "implementation class; mutate_inactive does NOT check whether its target is "
    "actually inactive despite its name, so do not rely on it for phenotype-safety. "
    ...
    "TOOLS BUCKET: two additional edit_type values, insert_tool and remove_tool, edit "
    "tool_registry.custom directly (pass tool_entry; other fields ignored) -- this is how "
    "the previously-inexpressible tools direction is expressed. insert_tool accepts a "
    "module path or a file:// URI to a tool file you author (a @tool-decorated async "
    "function), import-preflights it, and a custom tool with the same name as a builtin "
    "(e.g. a POSIX Bash replacing the cmd.exe Bash) OVERRIDES it at load with a logged "
    "warning. remove_tool only removes existing custom entries; builtins are overridden, "
    "never removed. These edits derive bucket=tools, do not change the genotype hash, and "
    "commit atomically with any graph edits in the same group."
)
_EDIT_ITEM_SCHEMA = {
    "type": "object",
    "properties": {
        "edit_type": {"type": "string", "enum": sorted([t.value for t in GraphEditType] + list(_TOOL_EDIT_TYPES))},
        ...
        "tool_entry": {
            "type": "string",
            "description": (
                "insert_tool / remove_tool only: a tool_registry.custom entry -- a module path "
                "(pkg.mod.tool_fn) or a file:// URI to a model-authored @tool file "
                "(file:///abs/path/my_tool.py::my_tool_fn). insert_tool import-preflights the "
                "entry; a custom tool named like a builtin (e.g. Bash) OVERRIDES it at load."
            ),
        },
    },
    "required": ["edit_type"],
...
_MANIFEST_DESC = (
    ...
    "infeasibility_note: when the previous Critic's strategy_concern flags a bucket your "
    "edits do not target, IV-11 refuses the candidate UNLESS the body carries a '## Why "
    "flagged direction is infeasible' section -- pass that section's text here (concrete "
    "evidence, not preference) and it is rendered for you. This matters because some "
    "flagged buckets (e.g. tools) cannot be expressed through graph edits at all."
)
_SCHEMA_MANIFEST = {
    "type": "object",
    "properties": {
        ...
        "infeasibility_note": {
            "type": "string",
            "description": (
                "Body text for '## Why flagged direction is infeasible' (IV-11 escape "
                "hatch when your edits do not target a strategy_concern-flagged bucket)."
            ),
        },
        "notes": {"type": "string"},
    },
...
class ProposalSession:
    ...
    def make_tools(self) -> list:
        ...
        async def graph_proposal_manifest(
            ...
            return session._manifest(
                ...
                infeasibility_note=infeasibility_note,
                notes=notes,
            )
    ...
    def _edit(self, candidate_id: str, edits: list, reason: str = "") -> dict:
        ...
        try:
            for raw in edits:
                et = raw.get("edit_type") if isinstance(raw, dict) else None
                if et in _TOOL_EDIT_TYPES:
                    entry = str((raw.get("tool_entry") or "")).strip()
                    if not entry:
                        raise _EditConversionError(
                            f"{et} requires a non-empty tool_entry (module path or file:// URI)"
                        )
                    tool_ops.append({"edit_type": et, "tool_entry": entry, "reason": reason})
                else:
                    group.append(_edit_from_dict(raw, reason))
            # Validate tool ops against the candidate's EFFECTIVE custom list
            # (parent custom + this candidate's prior tool edits), in order.
            if tool_ops:
                effective = self._effective_custom(candidate)
                for op in tool_ops:
                    if op["edit_type"] == "insert_tool":
                        if op["tool_entry"] in effective:
                            raise _EditConversionError(
                                f"insert_tool: {op['tool_entry']!r} is already in tool_registry.custom"
                            )
                        op["tool_name"] = _resolve_tool_entry(op["tool_entry"])
                        effective.append(op["tool_entry"])
                    else:
                        if op["tool_entry"] not in effective:
                            raise _EditConversionError(
                                f"remove_tool: {op['tool_entry']!r} is not in tool_registry.custom "
                                f"(current entries: {effective or '[]'}). Builtin tools cannot be "
                                "removed here -- override one by insert_tool-ing a custom tool with "
                                "the same name instead."
                            )
                        try:
                            op["tool_name"] = _resolve_tool_entry(op["tool_entry"])
                        except _EditConversionError:
                            op["tool_name"] = _tool_entry_stem(op["tool_entry"])
                        effective.remove(op["tool_entry"])
        ...
        old_tool_edits_len = len(candidate.tool_edits)
        ...
        candidate.tool_edits.extend(tool_ops)
        ...
        if not ok:
            ...
            del candidate.tool_edits[old_tool_edits_len:]
    ...
    def _manifest(
        ...
        *,
        ...
        #
        # Zero edits is the most unlandable a candidate gets, and it was the one
        # case the check below let through — its condition requires edits_log to
        # be non-empty, so "some edits, none of which land" was refused while
        # "no edits at all" sailed past. M16_L2_ghx5 R3 shipped two of these to
        # the Critic and lost the round.
        if not candidate.edits_log:
            return {
                "ok": False,
                "error": (
                    f"{candidate_id} has no applied edits, so its config is byte-identical "
                    "to the round parent and there is nothing for AEGIS compose to carry. "
                    "Call GraphProposalEdit at least once, then call this again. Until an "
                    "edit lands the manifest stays a draft in the candidate's scratch dir "
                    "and the Critic never sees it — drafting alone is not a deliverable "
                    "here, whatever the base prompt says about drafting manifests early."
                ),
            }
        ...
        if infeasibility_note is not None:
            # IV-11 escape hatch (M23): some flagged buckets (tools) cannot be
            # expressed through graph edits at all — without this section the L5
            # surface deadlocks against a strategy_concern it can never satisfy
            # (M23_L2 R7/R9/R13: three identical IV-11 deaths in a row).
            candidate.manifest_fields["infeasibility_note"] = infeasibility_note
            updated.append("infeasibility_note")
        ...
        # M23 shift-left: the commit-gate verdicts this candidate would meet are
        # reported HERE, synchronously, while the model can still fix them —
        # instead of two stages later in a file nobody reads (the IV-3 death of
        # M22 L1's top-ranked revert happened exactly there).
        split = self._predicted_split(candidate)
        if split is not None:
            ret["predicted_split"] = split
            if len(split["halo"]) + len(split["no_cone"]) > len(split["core"]):
                ret["warning"] = (
                    "predicted_impact is majority HALO/no-cone: most named tasks have no "
                    "edited node in their own cone. The Critic prices halo claims as "
                    "unsupported — trim the set, or make the edit reach those tasks' "
                    "cones (see graph_evidence/cone_sigs.json)."
                )
        from ..aegis.gates.structure import validate_candidate_manifest

        gate = validate_candidate_manifest(self._build_frontmatter(candidate), self._build_body(candidate))
        ret["structure_gate_ok"] = gate.ok
        if not gate.ok:
            ret["structure_gate_reason"] = gate.reason
        return ret
    ...
    def _status_for(self, candidate: _CandidateState) -> dict:
        ...
        split = self._predicted_split(candidate)
        if split is not None and len(split["halo"]) + len(split["no_cone"]) > len(split["core"]):
            checklist.append(
                "predicted_impact is majority halo/no-cone — trim the set or retarget "
                "the edit (cone discipline; the Critic prices halo claims as unsupported)"
            )
        if not checklist:
    ...
    def _render_config_yaml(self, candidate: _CandidateState) -> str:
        ...
        if candidate.tool_edits:
            tr = dict(base.get("tool_registry") or {})
            custom = list(tr.get("custom") or [])
            for op in candidate.tool_edits:
                if op["edit_type"] == "insert_tool" and op["tool_entry"] not in custom:
                    custom.append(op["tool_entry"])
                elif op["edit_type"] == "remove_tool":
                    custom = [e for e in custom if e != op["tool_entry"]]
            tr["custom"] = custom
            base["tool_registry"] = tr
        return yaml.safe_dump(base, allow_unicode=True, sort_keys=False)

    def _effective_custom(self, candidate: _CandidateState) -> list:
        """Parent's tool_registry.custom with this candidate's tool edits applied."""
        base = yaml.safe_load(self._parent_yaml_text) or {}
        custom = list(((base.get("tool_registry") or {}).get("custom")) or [])
        for op in candidate.tool_edits:
            if op["edit_type"] == "insert_tool" and op["tool_entry"] not in custom:
                custom.append(op["tool_entry"])
            elif op["edit_type"] == "remove_tool":
    ...
    # ── predicted-impact cone discipline (M23) ────────────────────────────────

    def _cone_sigs(self) -> dict:
        """This round's ``cone_sigs.json`` (written by the evidence materialiser),
        or ``{}`` when absent — absence reads as *no cone data*, never as an
        empty cone for every task."""
        path = self.candidates_dir.parent / "graph_evidence" / "cone_sigs.json"
        try:
    ...
    def _touched_static_ids(self, candidate: _CandidateState) -> set:
        """Static node ids this candidate's applied edits touch — targets, edge
        endpoints, and (for inserts) the minted ``proc:<slug>`` id."""
        ...
        for e in candidate.edits_log:
            for nid in (e.get("target_node_id"), e.get("edge_source_id"), e.get("edge_target_id")):
                if nid:
                    out.add(str(nid))
            spec = e.get("node_spec")
            if isinstance(spec, dict) and spec.get("_target_"):
                out.add(f"proc:{_compute_slug(str(spec['_target_']))}")
        return out

    def _predicted_split(self, candidate: _CandidateState) -> "dict | None":
        """Classify predicted tasks against their own cones: ``core`` (an edited
        node sits in the task's cone), ``halo`` (cone known, no touched node in
        it), ``no_cone`` (task has no cone signature this round).

        M22's measured failure shape: cluster-level lift evidence invited
        cluster-sized predictions (one ship named 27 tasks, hit 7) — nothing
        asked the per-task question. ``None`` when there is nothing to classify
        (no predicted tasks, or no cone_sigs on disk) — never a fabricated
        all-halo verdict.
        """
        pi = candidate.manifest_fields.get("predicted_impact")
        if not isinstance(pi, dict):
            return None
        tasks = sorted({str(t) for k in _PREDICTED_IMPACT_KEYS for t in (pi.get(k) or [])})
        if not tasks:
            return None
        failing = self._cone_sigs().get("failing") or {}
        if not failing:
            return None
        touched = self._touched_static_ids(candidate)
        core: list[str] = []
        halo: list[str] = []
        no_cone: list[str] = []
        for t in tasks:
            ...
            if sig is None:
                no_cone.append(t)
            elif touched & set(sig):
                core.append(t)
            else:
                halo.append(t)
        return {"core": core, "halo": halo, "no_cone": no_cone}
    ...
    def _build_frontmatter(self, candidate: _CandidateState) -> dict:
        ...
        # M23 cone discipline: the split is machine-derived and travels IN the
        # manifest so the Critic (and the ledgers) see it without new plumbing.
        split = self._predicted_split(candidate)
        if split is not None:
            fm["predicted_core"] = split["core"]
            fm["predicted_halo"] = split["halo"]
            if split["no_cone"]:
                fm["predicted_no_cone"] = split["no_cone"]
            candidate.derived_fields.add("predicted_split")
    ...
    def _build_body(self, candidate: _CandidateState) -> str:
        ...
        if body_text:
            ...
            lines.append("")
        note = str(candidate.manifest_fields.get("infeasibility_note") or "").strip()
        if note:
            # Heading text is IV-11's exact marker (aegis/gates/structure.py:217).
            lines.append("## Why flagged direction is infeasible")
            lines.append("")
            lines.append(note)
            lines.append("")
        return "\n".join(lines)

    def _draft_manifest_path(self, candidate: _CandidateState) -> Path:
        """Where a candidate's manifest lives before it has any edits.

        Beside its config in the scratch dir, NOT in ``candidates/`` — Stage 2
        globs ``candidates/C-R{n}-*.md``, so anything left there is a candidate
        as far as the Critic is concerned.
        """
        return candidate.config_path.parent / f"{candidate.candidate_id}.draft.md"

    def _write_manifest(self, candidate: _CandidateState) -> None:
        """Write the manifest — to ``candidates/`` only once an edit has landed.

        ``_open`` used to put it in ``candidates/`` straight away, so a candidate
        that was opened and never edited still reached the Critic as a real one.
        M16_L2_ghx5 R3 lost an entire round that way: two candidates, zero edits,
        ``capability_evidence: []``, a body this module itself had filled in with
        "call GraphProposalEdit ... before this candidate is reviewable" — and
        the Critic dutifully read both and returned no_op.

        The Evolver was not being careless. The vendored prompt tells it (P-1,
        ``templates/evolver.md``) to "draft manifests EARLY", that "a drafted
        manifest with thin evidence is reviewable", and that "an unwritten
        manifest is a wasted session"; ``capability_evidence: []`` is that
        instruction's own wording. Opening a candidate satisfied it, so the
        Evolver had every reason to believe it had delivered.

        Two instructions cannot both be obeyed, so the file placement decides
        which one is true: an unedited draft stays in the scratch dir where the
        Evolver can still read and finish it, and is invisible to Stage 2.
        """
        fm = self._build_frontmatter(candidate)
        fm_yaml = yaml.safe_dump(fm, allow_unicode=True, sort_keys=False)
        body = self._build_body(candidate)
        text = f"---\n{fm_yaml}---\n{body}"
        draft_path = self._draft_manifest_path(candidate)
        if candidate.edits_log:
            target, stale = candidate.manifest_path, draft_path
        else:
            target, stale = draft_path, candidate.manifest_path
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text(text, encoding="utf-8", newline="\n")
        # An edit promotes the draft; there is no path back, so the only stale
        # file to clear is the one on the other side of this call.
        try:
            stale.unlink()
        except OSError:
            pass
