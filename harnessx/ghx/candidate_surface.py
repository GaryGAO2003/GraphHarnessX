...
def derive_candidate_surface(
    ...
    # tools-bucket surface: diff tool_registry.custom between the raw yamls and
    # express the delta as `tool:<name>` node ids, so the scope gate's
    # removed→modify / added→additive ordering and cone-reach checks apply to
    # tool edits exactly as they do to processor edits. Best-effort — a
    # tool-surface failure must not sink the processor surface.
    try:
        t_added, t_removed = _tool_registry_delta(parent_config_path, candidate_config_path)
        nodes_added = sorted(set(nodes_added) | t_added)
        nodes_removed = sorted(set(nodes_removed) | t_removed)
...
def _tool_registry_delta(parent_config_path, candidate_config_path) -> "tuple[set, set]":
    """`tool:<name>` ids added/removed between the two configs' tool_registry.custom.

    Names resolve by importing the entry (the same resolution the loader uses);
    an unimportable entry degrades to its path/symbol stem — the id must exist
    even when the file is gone (the remove-after-delete case).
    """
    import yaml as _yaml

    def _custom(p) -> list:
        raw = _yaml.safe_load(Path(p).read_text(encoding="utf-8")) or {}
        return list(((raw.get("tool_registry") or {}).get("custom")) or [])

    def _tool_id(entry: str) -> str:
        try:
            from .graph_proposals import _resolve_tool_entry

            return f"tool:{_resolve_tool_entry(str(entry))}"
        except Exception:  # noqa: BLE001 — stem fallback keeps the id stable
            tail = str(entry).rsplit("::", 1)[-1].rsplit(".", 1)[-1].rsplit("/", 1)[-1]
            return f"tool:{tail or entry}"

    parent_custom = _custom(parent_config_path)
    cand_custom = _custom(candidate_config_path)
    added = {_tool_id(e) for e in cand_custom if e not in parent_custom}
    removed = {_tool_id(e) for e in parent_custom if e not in cand_custom}
