...
async def run_round_with_graph_evidence(
    ...
    **run_round_kwargs,
    ...
    is keyword-only, so ``round_n`` is always present).  ``failed_task_ids``,
    ``passed_task_ids`` and ``resolver`` drive
    :func:`~harnessx.ghx.evidence_files.materialize_graph_evidence` and are NOT forwarded
    to ``run_round``.  ``passed_task_ids`` is the control arm facts.md needs to report
    lift rather than bare failure frequency; passing it costs one cone computation per
    solved task and writes no extra file.  ``capability_resolver`` maps a task id to the
    judge's ``missing_capability`` payload; it feeds facts.md's absent-capability
    section, which is the half the cone cannot reach — a capability that does not exist
    is never invoked, so it has no node for lift to rank.  Omit it and that section is
    simply absent.
    ...
    # Pointer scope: the flip family failed this rollout too — their Digesters get
    # their cone (and, when present, the same-task diff file it anchors).
    pointer_scope = list(failed_task_ids or ()) + list(flip_task_ids or ())
    with install_brief_pointers(orchestrator.run_dir, run_round_kwargs["round_n"], pointer_scope):
        return await orchestrator.run_round(**run_round_kwargs)
