...
# The suppression above told the Evolver where manifests come from and stopped
# there, which left the base prompt's claim about WHAT COUNTS AS DONE standing.
# M16_L2_ghx5 R3 is what that costs: the Evolver opened two candidates, never
# edited either, and ended the round with two `capability_evidence: []` drafts —
# the base prompt's own wording — believing it had delivered, because there it is
# true that "a drafted manifest with thin evidence is reviewable" and that "an
# unwritten manifest is a wasted session". Under these tools it is not true, and
# saying only "the tools emit it" does not say so.
_DRAFT_IS_NOT_A_DELIVERABLE = (
    "Overrides the base prompt's 'Draft manifests EARLY' rule on ONE point: here a "
    "drafted candidate with no edits is NOT reviewable and is NOT a deliverable. "
    "GraphProposalOpen alone produces nothing the Critic will ever see -- the "
    "manifest stays a draft beside the candidate's config until GraphProposalEdit "
    "lands at least one edit, and only then moves into `candidates/`. "
    "GraphProposalManifest refuses outright while the edit count is zero. "
    "Drafting early is still right, and the rest of that rule still holds: open "
    "early, edit early, refine in place. What changed is that the edit -- not the "
    "draft -- is the point at which the candidate starts existing."
)
...
def _render_prompt_section(session: ProposalSession) -> str:
    lines = [
        ...
        "outstanding). Seven edit_type values: insert_node, remove_node,",
        "replace_same_group, change_dependency, swap_subgraph, mutate_inactive,",
        "mutate_params. To change an EXISTING processor's constructor kwargs",
        "(active or not), use mutate_params with node_changes={kwarg: value} --",
        "it refuses underscore metadata outright and its diff derives",
        "bucket=config. replace_same_group remains for swapping the class;",
        "mutate_inactive does not check whether its target is actually inactive",
        "despite its name.",
        "",
        "Cone discipline: GraphProposalManifest auto-classifies predicted_impact",
        "against each task's own cone (core = an edited node is in that task's",
        "cone; halo = it is not). The split is written into the manifest and the",
        "Critic prices halo claims as unsupported -- keep predictions core-heavy.",
        "",
        _METADATA_TRAP_WARNING,
        "",
        _LANDABILITY_RULE,
