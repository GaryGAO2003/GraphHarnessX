...
def _regression_diffs_path(run_dir, round_n: int) -> Path:
    return _evidence_dir(run_dir, round_n) / "regression_diffs.md"
...
def digester_evidence_paths(run_dir, round_n: int, task_id: str) -> "list[str]":
    ...
    # M23: when this task is one of the round's same-task P→F flips, its diff file
    # is the sharpest evidence available — point at it, but only for tasks it names.
    reg = _regression_diffs_path(run_dir, round_n)
    if reg.exists():
        try:
            if f"## {task_id}" in reg.read_text(encoding="utf-8"):
                out.append(str(reg.resolve()))
        except OSError:
            pass
    return out
...
# ── digester enrichment (M23) ──────────────────────────────────────────────────────


def _cone_causal_steps(cone_path: Path) -> "list[int]":
    """The step numbers a cone file names as causally mattering — parsed from the
    file this package itself renders (``evidence_files._render_cone``), so the two
    sides move together. Unparseable/absent reads as an empty list, never a guess."""
    try:
        text = cone_path.read_text(encoding="utf-8")
    except OSError:
        return []
    m = re.search(r"these are the steps to read:\s*\n([^\n]+)", text)
    if not m:
        return []
    return sorted({int(tok) for tok in re.findall(r"\d+", m.group(1))})


def _digester_pointer_md(run_dir, round_n: int, task_id: str, paths: "list[str]") -> str:
    """The Digester's pointer section: the path list plus the cone-first protocol.

    M22 measured the two failure shapes this text exists to close: only ~30% of
    failing-task digests ever opened their cone (the map was readable, the protocol
    was unstated), and IV-1 logged ~10 elided-path anchors per round per arm — the
    model abbreviating a long task-id path with a literal '...'. Both are
    instruction problems, so both fixes are instructions, placed where the digest
    is written rather than where it is validated.
    """
    section = _render_pointer_section(paths)
    extras: list[str] = []
    steps = _cone_causal_steps(_cone_path(run_dir, round_n, task_id))
    if steps:
        extras.append(
            "Cone-first protocol: the cone already names the steps that causally "
            f"mattered — steps {', '.join(str(s) for s in steps)}. Read THOSE steps in "
            "this task's trajectories/ file first; widen only if they do not explain "
            "the outcome."
        )
    extras.append(
        "When you cite an anchor, write the COMPLETE file name with the full task id "
        f"— e.g. `trajectories/{task_id}_r0.jsonl#step_3` — never abbreviate the id "
        "or the path with '...'. An elided anchor fails IV-1 validation and the "
        "citation is silently lost to every downstream reader."
    )
    return section + "\n" + "\n\n".join(extras) + "\n"
            ...
            if task_id is not None and str(task_id) in failed:
                ...
                if paths:
                    _append_pointer_to_config(
                        cfg, _digester_pointer_md(run_dir, round_n, str(task_id), paths)
                    )
