...
class GraphEditType(str, Enum):
    ...
    # M23: the campaign's most-wanted move — a parameter change on an existing
    # (active) processor — previously needed replace_same_group with a full
    # node_spec re-statement, which is where config-bucket candidates kept dying.
...
def _apply_one(snapshot: GraphSnapshot, edit: GraphEdit) -> None:
    ...
    elif edit.edit_type == GraphEditType.MUTATE_PARAMS:
        _apply_mutate_params(snapshot, edit)
...
def _apply_mutate_params(snapshot: GraphSnapshot, edit: GraphEdit) -> None:
    """Change constructor kwargs on an EXISTING node, active or not.

    The honest sibling of MUTATE_INACTIVE: no activity claim in the name, and no
    replace_same_group node_spec ceremony. Only constructor kwargs may change —
    underscore-prefixed metadata (``_hook_``/``_order_``/``_singleton_group_``/
    ``_after_``) is refused because those fields do not survive the S4 build
    fixed point unless the target class itself declares them (the metadata trap
    the L5 prompt documents). Refusing here keeps that trap structurally
    unreachable through this op.
    """
    nid = edit.target_node_id
    if nid not in snapshot.nodes:
        raise GraphEditError(f"MUTATE_PARAMS: node '{nid}' not found")
    if not edit.node_changes:
        raise GraphEditError("MUTATE_PARAMS requires a non-empty node_changes dict")
    bad = sorted(k for k in edit.node_changes if str(k).startswith("_"))
    if bad:
        raise GraphEditError(
            f"MUTATE_PARAMS changes constructor kwargs only; refused underscore "
            f"metadata {bad} — it would not survive the S4 rebuild unless the "
            f"target class itself declares the new value"
        )

    node = snapshot.nodes[nid]
    new_meta = dict(node.metadata)
    # Kwargs live both as plain metadata keys and (when present) inside the
    # _ctor_kwargs_ mirror — update both so every reader sees the same value.
    ctor = new_meta.get("_ctor_kwargs_")
    if isinstance(ctor, dict):
        merged = dict(ctor)
        merged.update(edit.node_changes)
        new_meta["_ctor_kwargs_"] = merged
    new_meta.update(edit.node_changes)
    snapshot.nodes[nid] = Node(
        node_id=nid,
        node_type=node.node_type,
