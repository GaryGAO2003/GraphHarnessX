...
def validate_edit_preconditions(
    ...
    for i, edit in enumerate(edits):
        ...
        if et is GraphEditType.MUTATE_PARAMS:
            if not isinstance(edit.node_changes, dict) or not edit.node_changes:
                _bad(i, "missing_node_changes", "MUTATE_PARAMS requires a non-empty node_changes dict")
            else:
                under = sorted(k for k in edit.node_changes if str(k).startswith("_"))
                if under:
                    _bad(
                        i, "underscore_params",
                        f"MUTATE_PARAMS changes constructor kwargs only; refused: {under} "
                        "(underscore metadata does not survive the S4 rebuild)",
                    )
