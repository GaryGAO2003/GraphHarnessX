...
class OpenAIProvider(BaseModelProvider):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert_gateway_base_url(base_url, "OpenAIProvider")
        self.model = model
    ...
    async def complete(
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                # Shared with LiteLLMProvider. Keeping a second copy here is what
                # let the two drift: this one had no notion of a network-level
                # failure, so APIConnectionError ("Connection error." and nothing
                # else — no status, no keyword) was judged non-retryable and killed
                # the rollout on attempt 1. One blip cost six rollouts across two
                # arms, unevenly, which biases an arm comparison directly.
                status, is_retryable = classify_retryable(e)
                err_str = str(e).lower()
                # Timeouts additionally get a longer backoff. Gateways spell it both
                ...
                if is_retryable and attempt < max_retries:
                    ...
                    # The give-up is logged because the retry line above only fires
                    # while ``attempt < max_retries`` — the LAST failure never printed
                    # one, so an exhausted budget was indistinguishable in a log from
                    # an error that was never retried at all.  Reading "no attempt 6/6
                    # line" as "no retry happened" is the exact mistake that costs a
                    # campaign its diagnosis.
                    logger.warning(
                        "API %s giving up after attempt %d/%d (%s)",
                        status or e,
                        attempt,
                        max_retries,
                        "retries exhausted" if is_retryable else "not retryable",
                    )
                    raise
    ...
    async def _complete_streaming(self, client, oai_messages, kwargs, max_retries, base_delay):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                # Same shared predicate as the non-streaming path above — this copy
                # had the identical blind spot for network-level failures.
                status, is_retryable = classify_retryable(e)
                err_str = str(e).lower()
                if status == 400 and ("flagged" in err_str or "content_policy" in err_str) and attempt == 1:
                    is_retryable = True
                if is_retryable and attempt < max_retries:
                    ...
                    # Same reason as the non-streaming path: without this the final
                    # failure leaves no line, and an exhausted retry budget looks
                    # identical to an error nothing ever retried.
                    logger.warning(
                        "API %s giving up after attempt %d/%d (%s)",
                        status or e,
                        attempt,
