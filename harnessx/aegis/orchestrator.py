...
def _strip_volatile_keys(cfg: dict) -> dict:
    ...
    # Canonicalisation stamps bookkeeping onto every processor entry. Those keys
    # differ between a hand-written parent and a canonicalised merged file for
    # reasons that have nothing to do with a candidate landing, so leaving them in
    # lets the empty-ship guard below pass on metadata alone — which is how L5's R1
    # shipped nothing and was not caught, while R2 hit the same bug loudly.
    for entry in out.get("processors") or []:
        if isinstance(entry, dict):
            for k in ("_code_hash", "_hooks_", "_order_", "_singleton_group_",
...
def _build_shipped_entries(
    *,
    round_dir: Path,
    round_n: int,
    shipped_cids: list[str],
    manifests_by_cid: dict[str, dict],
) -> list[tuple[str, str | list[str], Path]]:
    """Turn shipped cids into compose inputs, refusing to lose one on the way.

    A candidate is usable only if its applied config exists and it declared a
    bucket. Anything else used to be filtered out here in silence — and since
    this filter decides whether compose runs at all,
    ``_assert_merged_differs_from_base`` never saw it: with every candidate
    dropped there is no merged.yaml to compare, while ``shipped_cids`` stays
    non-empty and the ship is recorded downstream as landed.

    No live specimen: L2_103x10 R1 looked like one — decision.md ranking two
    candidates, no merged.yaml, no config referencing R1/applied — but the
    journal records ``action: no_op`` with ``decision cites unknown candidate
    C-R1-01 (IV-6)``, so the gate refused it correctly and nothing was lost.
    Both branches here are probably unreachable for the same reason: Stage 2
    (``propose.py:110``) drops a candidate with no applied config, and a missing
    manifest breaks the decision chain before shipped_cids is formed.

    Kept anyway. It costs nothing when unreachable, and the sibling assertion
    already refuses to proceed when compose ran and changed nothing — compose
    not running at all should not be the quieter of the two.
    """
    entries: list[tuple[str, str | list[str], Path]] = []
    dropped: list[str] = []
    for scid in shipped_cids:
        raw_bucket = (manifests_by_cid.get(scid) or {}).get("bucket", "")
        # Preserve list[str] for cross-bucket bundles (e.g. bucket: [prompt,
        # processor]). Earlier code coerced via str(), which turned a list into
        # "['prompt', 'processor']" — a non-existent bucket name that compose
        # silently skipped, leaving merged.yaml ≡ base.
        ...
        if applied_path.exists() and bucket:
            entries.append((scid, bucket, applied_path))
            continue
        why = []
        if not applied_path.exists():
            why.append("no applied/config.yaml")
        if not bucket:
            why.append("empty bucket")
        dropped.append(f"{scid} ({'; '.join(why)})")
    if dropped:
        raise ShipNotLandedError(
            f"round {round_n}: shipped_cids={shipped_cids} but {len(dropped)} "
            f"never reached compose: {', '.join(dropped)}. They would be recorded "
            f"as shipped while the round ran the parent config unchanged."
        )
    return entries
...
class AegisOrchestrator:
    ...
    # Step ceiling for the whole evolve phase — Planner, Evolver, Critic and the
    # Critic's ask-more runner all take it. 200 is the official value; it is the constraint that
    ...
    async def run_round(
        ...
        *,
        ...
        # An Evolver that produced nothing is not a Critic failure, and the
        # round must not say it was. ok_count was never read, so the empty list
        # went to Stage 3, the Critic spent its budget judging zero candidates,
        # and the round landed in the `critic_failed` branch below with
        # narrative="Critic failed" — a diagnosis whose remedy (more Critic
        # budget) is the opposite of the real one (a narrower Evolver brief).
        # narrative is read directly by the next Planner, so it is handed
        # forward as fact. L2_103x10 R5 and R9 both went this way (P-22).
        if not stage_2["candidate_paths"]:
            why = "; ".join(
                f"{cid}: {reason}" for cid, ok, reason in stage_2["results"] if not ok
            )
            narrative = (
                "Evolver produced zero candidates — the Critic was not run. "
                + (f"Stage 2 rejected: {why}" if why else "No candidate files were written at all.")
            )
            logging.getLogger("aegis.orchestrator").warning("round %d: %s", round_n, narrative)
            self._finalize_round(
                round_n, shipped_cids=[], hit_rate=None, narrative=narrative, refuted_signatures=[]
            )
            return {"shipped_cid": None, "shipped_cids": [], "reason": "no_candidates"}
        ...
        stage_3 = await run_stage_3(
            ...
            # P-25(b): Stage 2's drop reasons were produced and thrown away;
            # hand them to the Critic so it stops verdicting corpses.
            dropped_candidates=[
                (cid, reason) for cid, ok, reason in stage_2["results"] if not ok
            ],
        )
        self.audit.append(
            AuditEvent(
            ...
            self.audit.append(
                AuditEvent(
                    ...
                    payload={
                        ...
                        # Every non-empty reason, not just the failures. A gate
                        # that skips returns ok=True with "skipped: ...", so
                        # dropping the reason made a skip indistinguishable from
                        # a pass — counterfactual has two such paths and replay
                        # has one, and all three read as `true` here. A gate
                        # nobody can confirm ran is not a gate (P-24a). A clean
                        # pass carries no reason and stays quiet.
                        "reasons": {k: v.reason for k, v in gr.items() if v.reason},
                    },
                    evidence_refs=[],
                )
            )
        ...
        from .gates.structure import _normalize_bucket
        ...
        for cid, (manifest_path, _cfg_path) in candidates_info.items():
            ...
            try:
                ...
                logging.getLogger("aegis.orchestrator").warning(
                    "manifest read failed for %s (non-fatal): %s", cid, exc
                )

        # Compose BEFORE anything is recorded. A candidate compose cannot
        # express is dropped here, so recording first would credit ships that
        # are absent from the config the round actually runs — which is what
        # M15_smoke_L2 R1 did: ship_outcomes held three ships for a round that
        # composed none of them.
        merged_applied_path, shipped_cids, compose_rejected = self._compose_shipped(
            round_n=round_n,
            round_dir=round_dir,
            current_config_path=current_config_path,
            shipped_cids=shipped_cids,
            manifests_by_cid=manifests_by_cid,
        )
        shipped_set = set(shipped_cids)

        # The structure gate has accepted a list bucket since v0.9.3
        # (cross-bucket bundle), and this call site had not caught up: a list is
        # an unhashable dict key, so record() raised straight into a swallowing
        # except and the ship was never counted. Every M14_L2_arm candidate
        # declared `bucket: [tools]`, which is how that arm finished a whole
        # campaign with an empty reputation.json — its Planner saw a flat
        # unknown-boost every round while tools had already shipped twice
        # (P-15a).
        for cid in shipped_cids:
            for b in _normalize_bucket((manifests_by_cid.get(cid) or {}).get("bucket")):
                self.reputation.record(b, hit=True)
        ...
        for scid in shipped_cids:
            ...
            self.scoreboard.add_ship(
                ShipRecord(
                    cid=scid,
                    round=round_n,
                    # str() on a list yields "['tools']", a rollup key that
                    # never aggregates with anything (P-15a).
                    bucket=",".join(_normalize_bucket(fm.get("bucket"))),
        ...
        try:
            ...
            try:
                ...
                pass
            # A candidate compose refused is a rejection too, and its message
            # names the correct bucket. Until now that sentence reached only a
            # log file: the Evolver filed StepCountdownProcessor under `config`
            # in M14_L0_arm R3 and again in every round of M15's smoke, was
            # refused each time, and burned a candidate slot each time, because
            # nothing carried the correction back. rejected_candidates.jsonl is
            # already in the Critic's read scope (critic.py:259), so the reason
            # goes into that cid's existing row rather than a second one — a
            # compose-refused candidate is also a non-shipped sibling, and two
            # rows for it, one of them blank, is how it would read otherwise
            # (P-19).
            compose_reason = dict(compose_rejected)
            for cid, fm in manifests_by_cid.items():
                if cid in shipped_set:
                    continue
                if cid in compose_reason:
                    excerpt = f"compose refused this candidate: {compose_reason[cid]}"
                else:
            ...
            # Note which earlier rejections this round picked back up. The
            # function existed the whole time and nothing ever called it, so
            # rejected_candidates.jsonl — read by the Critic, the Evolver and
            # the Planner — could never say whether an archived idea came back
            # (P-23). Runs after the append so this round's own rejections are
            # in the set being matched against.
            ledger.backfill_rejected_revivals(self.run_dir)
        except Exception as exc:
            logging.getLogger("aegis.orchestrator").warning(
    ...
    def _compose_shipped(
        ...
        *,
        ...
        """Merge the shipped candidates and report which ones actually landed.

        Runs before the round records anything. A candidate compose cannot
        express is dropped on its own now (P-18), so ``shipped_cids`` and what
        landed can differ, and the ledger must be told the second number.
        Recording first is how M15_smoke_L2 R1 ended up with three ships in
        ship_outcomes for a round that composed none of them.

        Returns ``(merged_path_or_None, landed_cids, rejected)``, where
        ``rejected`` is ``[(cid, why)]`` for candidates compose refused. The
        caller writes those into the rejected-candidates ledger: the refusal
        text names the correct bucket, and until P-19 it reached only a log
        file, so the Evolver re-proposed the identical mis-bucketed candidate
        every round.
        """
        if not shipped_cids:
            return None, shipped_cids, []
        from .compose import compose_shipped_configs

        shipped_entries = _build_shipped_entries(
            round_dir=round_dir,
            round_n=round_n,
            shipped_cids=shipped_cids,
            manifests_by_cid=manifests_by_cid,
        ...
        composed = compose_shipped_configs(
        ...
        if composed.rejected:
            logging.getLogger("aegis.orchestrator").warning(
                "round %d: %d of %d shipped candidate(s) could not be composed and "
                "were dropped: %s",
                round_n,
                len(composed.rejected),
                len(shipped_cids),
                "; ".join(f"{cid}: {why}" for cid, why in composed.rejected),
            )
            landed = set(composed.landed)
            shipped_cids = [c for c in shipped_cids if c in landed]
        if composed.output_path is None:
            # Every candidate rejected: nothing written, so there is no config
            # change to assert on. The round runs the parent config and the
            # ledger's not_scoreable path (P-16) covers it.
            return None, shipped_cids, composed.rejected
        # Hard safety: a successful ship MUST mutate the config. If merged ≡
        # base after compose, an applier silently no-op'd (e.g. unknown bucket
        # name). Refuse to proceed so the round runs on a config that actually
        # reflects the ship, instead of repeating the parent and being mistaken
        # for variance.
        ...
        return merged_applied_path, shipped_cids, composed.rejected
