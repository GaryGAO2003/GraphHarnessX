...
def make_evolver_runner(
    ...
    async def runner(cid: str, question: str) -> str:
        ...
        result = await harness.run(BaseTask(
            ...
            max_steps=max_steps, max_cost_usd=100.0,
        ))
        return result.final_output or "(no answer)"
    return runner


def _dropped_note(dropped: "list[tuple[str, str]] | None") -> str:
    """One paragraph telling the Critic what Stage 2 already threw away.

    Stage 2 has always known which candidates it dropped and why — the reasons
    go to audit.jsonl, which no agent reads. The Critic gets the raw
    candidates/ directory, so it spends verdict budget on the corpses and then
    cites them, and the citation used to void the whole round (P-25).
    """
    if not dropped:
        return ""
    lines = "\n".join(f"- {cid}: {str(reason)[:200]}" for cid, reason in dropped)
    return (
        "\n\nStage 2 DROPPED these candidate(s) before validation — they have no "
        "usable manifest this round. Do NOT write verdicts for them and do NOT "
        "put them in ship_ranking; a citation to one of them is stripped at the "
        "gate:\n" + lines
    )
...
async def run_stage_3(
    *,
    ...
    # The Critic is the stage most at risk from raising the Evolver alone: it ran
    # 153 of 200 steps judging four candidates, and a roomier Evolver produces more
    # of them. A round that dies at judgment with candidates in hand is worse than
    # one that never produced any.
    max_steps: int = 200,
    meta_sessions_dir: Path | None = None,
    dropped_candidates: "list[tuple[str, str]] | None" = None,
) -> dict:
    ...
    result = await harness.run(BaseTask(
        description=(
            ...
            f"delimited by '---'."
            + _dropped_note(dropped_candidates)
        ),
