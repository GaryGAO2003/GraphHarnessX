...
- ``data/rejected_candidates.jsonl`` — one line per rejected candidate
  ...
  ``revived_as`` field when a later candidate's ``iterates_from`` names it,
  which is how a rejected idea coming back becomes countable (P-23).
...
# How much of a task id has to agree before a divergence counts as a typo rather
# than a different task. Eight hex characters over a bed of ~100 UUIDs collide
# with probability around 1e-7.
_MIN_ID_PREFIX = 8
...
def resolve_task_ids(run_root: Path, ids: list[str], *, where: str = "") -> list[str]:
    """Map whatever the Evolver typed onto real task_history keys.

    ``predicted_tasks`` is free text: the manifest field is copied through
    ``str(x)`` with no validation, and the backfill then matches it exactly
    against task_history. A near-miss therefore does not fail — it silently
    grades every predicted task ``unknown`` and reports ``hit_rate: 0/N``.

    M14 ran the whole way like that. ``M14_L0_arm``'s Evolver wrote 8-char
    truncated IDs (``33d8ea3b`` for ``33d8ea3b-6c6b-4ff1-803d-7e270dea8a57``),
    so both of its ships scored a fabricated 0/13 and 0/3 and the next round's
    Planner reasoned off those zeros. The full IDs were available to it the
    whole time — ``digests/`` filenames and ``landscape.md`` carry them — the
    two forms simply coexist on the doc plane with nothing checking which one
    lands in the ledger.

    So: exact hits pass through, a unique prefix expands to the full ID, and
    anything that resolves to nothing or to more than one task is kept as-is
    and named in a warning. Never silent, never wrong-by-guess.
    """
    known = {row.get("task_id") for row in read_task_history(run_root)}
    known.discard(None)
    if not known:
        return list(ids)
    out: list[str] = []
    ...
    for raw in ids:
        ...
        if tid in known:
            out.append(tid)
            continue
        matches = sorted(k for k in known if k.startswith(tid))
        if len(matches) == 1:
            out.append(matches[0])
            expanded.append(f"{tid}->{matches[0]}")
            continue
        if not matches:
            # Third shape (P-20): the right prefix with an invented tail, which
            # prefix expansion cannot reach because the typed id is not a prefix
            # of anything — it is the same length and forks midway. M16_L0_ghx0
            # wrote 7673d772-ef80-4f0a-... for 7673d772-ef80-4f0f-...: one
            # character, and both of that arm's ships carried it.
            #
            # Only when the prefix expansion above found nothing, so an ambiguous
            # abbreviation stays ambiguous rather than being resolved by a
            # longer common run. Eight hex characters over ~100 UUIDs collide
            # with probability ~1e-7; below that the agreement is not evidence
            # the model was copying a real id.
            scored = sorted(((_common_prefix_len(k, tid), k) for k in known), reverse=True)
            best = scored[0][0] if scored else 0
            winners = [k for n, k in scored if n == best]
            if best >= _MIN_ID_PREFIX and len(winners) == 1:
                out.append(winners[0])
                corrected.append(f"{tid}->{winners[0]} (diverges at char {best})")
                continue
        out.append(tid)
        unresolved.append(f"{tid} ({len(matches)} matches)")
    tag = f" [{where}]" if where else ""
    if expanded:
        _log.warning(
            "ledger%s: expanded %d abbreviated task id(s) to full ids: %s",
            tag,
            len(expanded),
            ", ".join(expanded),
        )
    if corrected:
        # Kept separate from `expanded`: an abbreviation is the same id written
        # short, while this is an id the model got wrong. Same repair, different
        # thing to know about the writer.
        _log.warning(
            "ledger%s: %d predicted task id(s) matched a real task only up to a "
            "point and were invented past it — repaired by longest common prefix: %s",
            tag,
            len(corrected),
            ", ".join(corrected),
        )
    if unresolved:
        _log.warning(
            "ledger%s: %d predicted task id(s) match no task in history — they will "
            "grade as `unknown` and drag hit_rate down: %s",
            tag,
            len(unresolved),
            ", ".join(unresolved),
        )
...
def record_ship_outcome(
    ...
    *,
    ...
    entry = {
        ...
        # Canonicalised on the way in, so ship_outcomes.json never stores an
        # id the backfill cannot match (P-15b).
        "predicted_tasks": resolve_task_ids(
            run_root, list(predicted_tasks), where=f"R{round_n} {shipped_cid}"
        ),
...
def backfill_ship_outcomes(run_root: Path) -> Path:
    ...
    for entry in outcomes:
        ...
        # A ship the Critic accepted but compose never landed must not be
        # scored. The round still ran rollouts — on the PREVIOUS round's config,
        # carried forward — so comparing R(n-1) to R(n) measures a same-config
        # repeat and reports it as this ship's hit rate. M14's last round is the
        # live case: decision_type: ship, two candidates accepted, no
        # merged.yaml, and R3/config.yaml referencing only R2's applied dirs.
        # Scored anyway it reads 2/2 and 4/4 — a flawless record invented out of
        # noise, which is exactly what the next Planner would have built on.
        # Decidable only when compose left its output directory behind: M14's R3
        # has applied/C-R3-01 and applied/C-R3-02 but no merged.yaml, which is
        # candidates materialised and nothing merged. A run root with no
        # applied/ at all says nothing either way, and absence of evidence must
        # not become evidence of a dropped ship.
        applied_dir = run_root / f"R{ship_round}" / "applied"
        if applied_dir.is_dir() and not (applied_dir / "merged.yaml").exists():
            entry["hit_rate"] = None
            entry["hit_rate_strict"] = None
            entry["not_scoreable"] = (
                f"no R{ship_round}/applied/merged.yaml — the ship was accepted but "
                f"never composed, so R{ship_round}'s rollouts ran the previous "
                f"config and there is nothing here to attribute"
            )
            _log.warning(
                "ledger [R%d %s]: accepted but never composed — leaving hit_rate "
                "unscored rather than crediting a same-config repeat",
                ship_round,
                entry.get("ship_id", "?"),
            )
            continue
        entry.pop("not_scoreable", None)
        # Also on the way out: rows written before P-15b, or by a path that
        # bypassed record_ship_outcome, still heal here rather than reporting
        # a confident zero.
        preds = resolve_task_ids(
            run_root,
            entry.get("predicted_tasks", []) or [],
            where=f"R{ship_round} {entry.get('ship_id', '?')}",
        )
        entry["predicted_tasks"] = preds
...
# The retired briefs model expressed revival as `lead_pointer: archive:<cid>`;
# the live one is the candidate manifest's `iterates_from` (P-23).
_ITERATES_FROM_RE = re.compile(r"^\s*iterates_from\s*:\s*(\S+)", re.MULTILINE)
...
def backfill_rejected_revivals(run_root: Path) -> Path:
    """Note which rejected candidates a later round picked back up.

    Rejection is usually "not now" — wrong bucket, thin evidence, bad timing —
    rather than "never", so whether an archived idea comes back, and whether the
    second attempt works, is worth being able to answer. rejected_candidates.jsonl
    is in the read scope of the Critic, the Evolver and the Planner, so the answer
    reaches all three.

    This used to scan ``R<n>/briefs/B-R<n>-NN.md`` for ``lead_pointer: archive:<cid>``
    and could never fire: that dispatch model was retired (orchestrator.py: "No
    brief-to-candidate alignment any more ... there are no separate briefs"), the
    corpus holds zero briefs directories and zero B-R*.md files, and nothing ever
    called this function. It kept the old layout "for backward compatibility" with
    a layout that no longer existed.

    The live signal is the candidate manifest's ``iterates_from``. M16_L0_ghx0's
    R3/C-R3-02 declares ``iterates_from: C-R2-03``, and C-R2-03 is in the archive
    as rejected — one revival, which the old scan would have missed forever.

    Idempotent: only rewrites when something changed.
    """
    path = data_dir(run_root) / _REJECTED
    rows = _read_jsonl(path)
    if not rows:
        return path
    rejected_cids = {r.get("candidate_id") for r in rows}
    rejected_cids.discard(None)

    revivals: dict[str, list[dict]] = {}
    for cand in sorted(Path(run_root).glob("R*/candidates/*.md")):
        # R<n>/candidates/C-R<n>-NN.md — the round is the grandparent directory,
        # which beats a path regex that has to care whether the separator is / or \.
        round_dir = cand.parent.parent.name
        if not (round_dir.startswith("R") and round_dir[1:].isdigit()):
            continue
        try:
            head = cand.read_text(encoding="utf-8", errors="replace")[:4000]
        except OSError:
            continue
        m = _ITERATES_FROM_RE.search(head)
        if not m:
            continue
        src = m.group(1).strip().strip("\"'")
        if src not in rejected_cids or src == cand.stem:
            continue
        entry = {"round": int(round_dir[1:]), "candidate_id": cand.stem}
        bucket = revivals.setdefault(src, [])
