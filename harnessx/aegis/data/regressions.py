...
def render_regressions_md(
    ...
    *,
    ...
    for grade in ("regressed_hard", "regressed_soft", "regressed_partial"):
        ...
        if rows:
            # Full ids, not task_id[:8]. An agent reading this watchlist copies
            # what it sees into predicted_tasks, and the ledger matches those
            # exactly — M14_L0_arm's two ships both scored a fabricated 0 that
            # way. Nothing on the doc plane may show a task id an agent cannot
            # safely quote back (P-15b).
            ids = ", ".join(f"`{r['task_id']}`" for r in rows)
