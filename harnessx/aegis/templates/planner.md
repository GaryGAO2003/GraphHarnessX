...
{% if round >= 2 %}- **Prior Critic's strategy_concern (if any).** Read `R{{ round_minus_1 }}/decision.md`.
  ...
  **Also name the buckets in `strategy_concern_flagged_buckets` in your
  frontmatter** (one or more of `prompt` / `tools` / `config` / `processor`).
  The prose is for the Evolver to read; that field is what the sixth structural
  gate reads, and it is the only thing that makes the concern binding: a
  candidate must then either target one of those buckets or prove the direction
  infeasible with evidence. Relaying the concern in prose but leaving the field
  empty quietly demotes it back to advice, which is what it was before the gate
  existed.
{% endif %}
...
unattempted_directions:        # approaches not tried yet per ship_outcomes
  - <short description>
strategy_concern_flagged_buckets:   # REQUIRED when you relayed a strategy_concern
  - <bucket>                        # above; omit or leave empty only when there was none
---
