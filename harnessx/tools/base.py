...
def _truncate_result(result: ToolResult, tool_name: str) -> ToolResult:
    ...
    if not isinstance(output, str) or len(output) <= _MCP_TEXT_THRESHOLD:
        return result
    # Look for the marker in the TAIL only. The notice is appended at the end, so
    # that is the only place a previous truncation can have left one; scanning the
    # whole string turns the guard into a content sniff, and any output that merely
    # mentions the marker bypasses the cap entirely.
    #
    # That is not hypothetical. An Evolver ran a repo-wide grep, the result matched
    # this very line, and the guard waved through 5 MB — roughly 1.3M tokens — into
    # the conversation. Compaction could not recover: `retention_window` keeps recent
    # messages intact, so the giant one was pinned and a compaction pass took the
    # context from 2,776,876 tokens to 2,730,104. The next model call exceeded the
    # 512k window, the 400 is not retryable, and both the Evolver and the Critic died
    # with the round producing nothing.
    if "[truncated" in output[-300:]:
        return result
