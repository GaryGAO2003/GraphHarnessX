    ...
    *hook* is the logical hook name (e.g. ``"step_start"``, ``"after_model"``)
    ...
    async def process(  # type: ignore[override]
        ...
        for processor in self.processors:
            ...
            # v6 M13: the diff that decides "this processor actually did something" is
            # the same one the tracer reports, so it is computed once and both consume
            # it.  U records it on the invocation node — membership of a causal cone
            # then follows from what an invocation DID, not from where it sat in the
            # control chain.
            if tracer is not None or _cur_inv is not None:
                action = _diff_primary(prev, events, event_type)
                if action and _cur_inv is not None:
                    _unfold_rec.mark_intervention(_cur_inv, action)
                if action and tracer is not None:
