    ...
    if cfg.custom:
        ...
        for target in cfg.custom:
            try:
                ...
                except Exception as _conflict:
                    from ..tools.base import ToolConflictError

                    if not isinstance(_conflict, ToolConflictError):
                        raise
                    # A custom entry that shares a builtin's name is an
                    # intentional override (the tools-bucket evolution path:
                    # e.g. a POSIX Bash replacing the cmd.exe Bash) — replace,
                    # loudly, instead of dropping the custom tool.
                    registry.register(fn, replace=True)
                    _log.warning(
                        "tool_registry.custom: %r OVERRIDES the already-registered "
                        "tool of the same name (%s)",
                        target,
                        getattr(fn, "name", "?"),
                    )
    ...
    async def run(
        ...
        *,
        ...
        try:
            ...
            if unfold_enabled():
                from ..graph.unfold import UnfoldRecorder, recorder_for_run, unfold_accumulate_enabled

                _u_session = session_id or state.run_id
                # A conversational bed calls run() once per turn under one run id
                # (resume_state carries it), so a per-call recorder would make U the
                # last turn instead of the task. Opt-in; GAIA's one-call-per-task
                # shape is unchanged with the flag off.
                _unfold_rec = (
                    recorder_for_run(state.run_id, _u_session)
                    if unfold_accumulate_enabled()
                    else UnfoldRecorder(run_id=state.run_id, session_id=_u_session)
                )
