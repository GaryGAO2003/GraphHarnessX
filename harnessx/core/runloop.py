            ...
            for tc in model_event.tool_calls:
                ...
                try:
                    if tc_event.approved:
                        ...
                        # M23 (B3): stamp the tool invocation's outcome on its U node —
                        # error / empty / ok — so graph evidence can rank mechanism-shaped
                        # variants (tool:Bash#empty) instead of bare tool names.
                        # Recorder-gated: a run without U recording pays nothing here.
                        if _unfold_rec is not None and _tool_inv_id is not None:
                            # M24 fix #4: judge the FLATTENED payload, not the raw
                            # object — structured-block returns with no text used to
                            # grade ok while the model saw nothing (x6-28 undercount).
                            from harnessx.graph.unfold import classify_tool_outcome

                            _unfold_rec.annotate_outcome(
                                _tool_inv_id,
                                classify_tool_outcome(
                                    tr_event.result,
                                    error=tr_event.error,
                                    content_blocks=getattr(tr_event, "content_blocks", ()),
                                ),
                            )
