...
def main() -> None:
    ...
    for run in ALL:
        ...
        share[run] = c["tools"] / n
        print(f"  {run:15s} N {n:2d}  " + "  ".join(f"{k} {c[k]:2d}" for k in BUCKETS) + f"   tools share {share[run]*100:.1f}%")
    for arm in ("no-graph", "graph"):
        n = sum(pooled[arm].values())
        print(f"  {arm:9s} N {n}  " + "  ".join(f"{k} {pooled[arm][k]} ({pooled[arm][k]/n*100:.1f}%)" for k in BUCKETS)
              + f"   multi-bucket cards {multi[arm]}/{n}")
    ng = [share[r] for r in NO_GRAPH]
    g = [share[r] for r in GRAPH]
    obs = sum(g) / 3 - sum(ng) / 3
    vals = ng + g
    cnt = sum((sum(vals[i] for i in combo) / 3 - sum(vals[i] for i in range(6) if i not in combo) / 3) >= obs - 1e-12
              for combo in itertools.combinations(range(6), 3))
    print(f"  seed-level tools share: no-graph {[round(x*100,1) for x in ng]}  graph {[round(x*100,1) for x in g]}"
    ...
    for arm, runs in (("no-graph", NO_GRAPH), ("graph", GRAPH)):
        ...
        for run in runs:
            ...
            for _, cid, b, _ in cards[run]:
                tot[b[0]] += 1
                acc[b[0]] += cid in ok
        print(f"  Critic acceptance {arm:9s} " + "  ".join(f"{k} {acc[k]}/{tot[k]} ({acc[k]/max(tot[k],1)*100:.0f}%)" for k in BUCKETS))
    gate = collections.Counter(); labels = collections.Counter()
    for run in GRAPH:
        ...
        for gf in strict(RUNS / run, "R*/graph_evidence/gate/C-*.md"):
            ...
            key = ("refused" if ok and ok.group(1) == "False" else "unchecked" if checked and checked.group(1) == "False" else "passed")
            b = bucket_of.get(gf.stem, "?")
            gate[(b, key)] += 1
            if key == "refused":
                labels[label.group(1) if label else "?"] += 1
    print("  graph-existence gate, three graph seeds: " + "  ".join(
        f"{k} {gate[(k,'refused')]}R/{gate[(k,'passed')]}P/{gate[(k,'unchecked')]}U" for k in BUCKETS)
        + f"   refusal labels {dict(labels)}")
    for arm, runs in (("no-graph", NO_GRAPH), ("graph", GRAPH)):
        landed = collections.Counter(); n_ships = 0
        for run in runs:
            bucket_of = {cid: b[0] for _, cid, b, _ in cards[run]}
    ...
    for arm, runs in (("no-graph", NO_GRAPH), ("graph", GRAPH)):
        flags = collections.Counter(); comply = [0, 0]; fl = [0, 0]; other = [0, 0]
        for run in runs:
            fb = {rd: flagged_bucket(run, rd) for rd in range(1, WINDOW + 1)}
            flags.update(v for v in fb.values() if v)
            for rd, _, b, _ in cards[run]:
                if fb.get(rd):
                    comply[1] += 1; comply[0] += b[0] == fb[rd]
                slot = fl if fb.get(rd) == "tools" else other
                slot[1] += 1; slot[0] += b[0] == "tools"
        print(f"  flagged rounds {arm:9s} {dict(flags)}  cards in flagged rounds matching the flag {comply[0]}/{comply[1]}"
    ...
    CITE = {
        ...
        "cone / unfold / motif": re.compile(r"\bcone\b|\bunfold|\bmotif|empty_consumed|budget_no_commit|ungrounded_commit", re.I),
    ...
    for arm, runs in (("no-graph", NO_GRAPH), ("graph", GRAPH)):
        n = 0; tot = collections.Counter(); present = collections.Counter()
        for run in runs:
            for _, _, _, t in cards[run]:
                n += 1
                for k, rx in CITE.items():
                    c = len(rx.findall(t)); tot[k] += c; present[k] += c > 0
        ...
        for _, _, _, t in cards[run]:
            for m in OPS.findall(t):
                for name, k in re.findall(r"(\w+?)x(\d+)", m):
                    ops[name] += int(k)
    print(f"  typed operations, three graph seeds: {dict(ops.most_common())}  (never used: "
    ...
    for run in GRAPH:
        ...
        for f in strict(RUNS / run, "R*/graph_evidence/facts.md"):
            if rnum(f) > WINDOW:
                continue
            rows = sorted([(nm, float(l)) for nm, _, _, l in ROW.findall(read(f))], key=lambda x: -x[1])
            facts[rnum(f)] = rows
        n = top3 = top1 = 0
        for rd, _, b, t in cards[run]:
            ...
            if kind in by_top:
                by_top[kind][1] += 1; by_top[kind][0] += b[0] == "tools"
        print(f"  {run}: cards with a lift table {n}; naming a top-3 node {top3} ({top3/max(n,1)*100:.0f}%); naming the top-1 {top1} ({top1/max(n,1)*100:.0f}%)")
    print(f"  P(tools card | top-1 node is a tool) = {by_top['tool'][0]}/{by_top['tool'][1]};"
    ...
    for run in ALL:
        ...
        outside = sum(len(unlock_targets(t) - (id_block(t, "predicted_core") or set()) - (id_block(t, "predicted_halo") or set())
                          - (id_block(t, "predicted_no_cone") or set())) for _, _, _, t in cards[run] if id_block(t, "predicted_core") is not None)
        n = len(u)
        print(f"  {run:15s} cards {n:2d}  unlock/card mean {sum(u)/n:.1f} median {sorted(u)[n//2]} max {max(u)}"
    ...
    for arm, runs in (("no-graph", NO_GRAPH), ("graph", GRAPH)):
        ...
        print(f"  pooled {arm:9s} cards {n}  unlock/card {u/n:.2f}  core/card {core/n:.2f}  halo/card {halo/n:.2f}"
    ...
    for run in GRAPH:
        ...
        for k, cid in scoreboard_ships(run):
            ...
            for cls in ("core", "halo"):
                ...
                hit[cls][1] += len(elig); hit[cls][0] += sum(fresh[(k, x)] for x in elig)
            if (run, k) not in seen_rounds:
                seen_rounds.add((run, k))
                pool = [x for (r, x), v in fresh.items() if r == k - 1 and v is False and (k, x) in fresh]
                base[1] += len(pool); base[0] += sum(fresh[(k, x)] for x in pool)
    print(f"  three graph seeds, landed ships, F9 rule (fresh-failed at k-1, fresh at k, passes at k):"
            ...
            for s in strict(RUNS / run, "R*/summary.md"):
                ...
                for _t, pat, tag in IDX.findall(read(s)):
                    if pat != "ALL_PASS" and tag not in ("none", ""):
                        tags[tag] += 1
        n = sum(tags.values()); top3 = sum(v for _, v in tags.most_common(3)); once = sum(1 for v in tags.values() if v == 1)
        print(f"  {arm:9s} failing digests {n}  distinct tags {len(tags)}  used once {once} ({once/len(tags)*100:.0f}% of the vocabulary)"
    ...
    for run in ALL:
        ...
        print(f"  {run:15s} decisions {dict(dec)}  regression files {len(hard)}, with hard regressions {nz}, "
    ...
    for run in ALL:
        ...
        rows = [json.loads(l) for l in (RUNS / run / "data" / "rejected_candidates.jsonl").open(encoding="utf-8", errors="replace")]
        ...
        print(f"  {run:15s} tools cards {sum(names.values()):2d} over {len(names):2d} tool names, touched>=2 {multi_names}"
    ...
    for run in ALL:
        ...
        for rd, _, _, t in cards[run]:
            ...
            if not targets or not failing:
                continue
            fail_n += len(failing); fail_vol += sum(cls[x] == "volatile" for x in failing)
            aim_n += len(targets); aim_vol += sum(cls[x] == "volatile" for x in targets)
        if aim_n and fail_n:
            print(f"  {run:15s} targets {aim_n:3d}  aim %volatile {aim_vol/aim_n*100:5.1f}  failing-set %volatile {fail_vol/fail_n*100:5.1f}"
