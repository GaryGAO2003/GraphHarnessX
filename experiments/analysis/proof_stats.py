...
Cells: (task, batch) net-win difference. Dirty tasks (8b3379c0, 8131e2c0 鈥?
leak-route) excluded everywhere.
Primary:   sign test over nonzero cells 鈥?reported BOTH all-cells
...
def main() -> int:
    ...
    for c in cells:
        ...
        print(
            f"[{mark}] {c['batch']:>8} {c['tid']}  parent {c['pw']}/{c['pn']}  "
            f"applied {c['aw']}/{c['an']}  d={c['d']:+d}"
        )

    for name, sel in (
        ("ALL cells (retro-contaminated, descriptive)", cells),
        ("PROSPECTIVE only (pre-registered primary)", [c for c in cells if c["prospective"]]),
    ):
    ...
    print(
        f"mechanism: confident-wrong among fails  parent {mech['p_cw']}/{mech['p_f']}"
