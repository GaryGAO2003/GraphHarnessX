...
Bins (assignment priority order):
  leak-route   鈥?>=50% of PASS trajectories touch benchmark-artifact markers
  wall         鈥?>=50% of FAIL trajectories carry hard wall markers (403/503/
                 security verification/blocked) on page-grade fetches
  friction     鈥?fails dominated by Bash empty/error streaks (851e family)
  divergence   鈥?web-dominant, median fail calls >= pass calls + 6 (8b family)
  wrong-close  鈥?fails mostly exit=done with fail calls close to pass calls
  starved      鈥?fails mostly budget_exceeded at cap
  mixed        鈥?none dominant
Also: T2b 鈥?per-rep leak audit of DOSSIER6 _cov applied 8131 passes.
"""
...
LEAK = ("GAIA_web", "gaia_validation", "metadata.jsonl", "ground_truth", "Final_Assignment_Template", "adk-gaia")
WALL = (
    "HTTP 403",
    "HTTP Error 503",
...
def traj(tid, rnd):
    for p in (RUN / f"R{rnd + 1}" / "trajectories" / f"{tid}_r0.jsonl", RUN / f"R{rnd}" / "raw" / f"{tid}_r0.jsonl"):
        if p.exists():
            return p
    return None
...
def main() -> int:
    ...
    try:
        ...
        ROOT = require_path(
            runs_root(args.runs_root) / "PROBE_DOSSIER6/verify_sessions_applied", "applied verification sessions"
        )
    ...
    for tid, atts in sorted(hist.items()):
        ...
        pc = med([x[0] for x in p_feats])
        fc = med([x[0] for x in f_feats])
        fws = med([x[1] for x in f_feats])
        fb = med([x[2] for x in f_feats])
        ...
        bins[b].append(short)
        detail[short] = (
            f"P{np_}/{len(atts)} callsP{pc}/F{fc} WSf{fws} Bf{fb} "
