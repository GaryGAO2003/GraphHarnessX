...
localization. It is \textbf{not supported on declared substitute
endpoints}: its registered endpoint proved not measurable on this design,
and the reason that happened is itself the main result
(\S\ref{sec:readout}).
...
(\S\ref{sec:what-graph-did}). Fixed, with H1, before the graph campaign
ran: an instrument is trusted only once it is shown not to have changed
what it measures.

\paragraph{Q3, the effect of graph evidence.}\label{rq:3} Not detectable,
and on the registered endpoint not measurable at all. Pre-registered with
...
laws, the pre-registered efficacy trial --- that
