...
MIKTEX = Path(r"C:\Users\Admin\AppData\Local\Programs\MiKTeX\miktex\bin\x64")
PDFLATEX = MIKTEX / "pdflatex.exe"
PDFTOTEXT = MIKTEX / "pdftotext.exe"
...
HARD_BANNED = [
    ...
    r"\b1\.04\b", r"1,375", r"\\times\s*20\b", r"×\s*20", r"26--33",   # 1.99 is the whitelist M22-L0 lift (F-row), not banned
    r"twenty-fold", r"\bx20\b",
]
...
def check_compile(compile_: bool = True) -> dict:
    ...
    return {"status": "FAIL" if fails else "PASS", "pages": pages, "errors": len(errors),
            "overfull_hbox": len(overfull_h), "overfull_vbox": overfull_v,
            "seconds": round(time.time() - t0, 1), "fails": fails}
...
def check_abstract() -> dict:
    ...
    return {"status": "PASS" if ok else "FAIL", "page2": p2, "page3": p3,
...
def check_exclusion() -> dict:
    ...
    return {"status": "FAIL" if fails else "PASS", "hard_hits": hard_hits, "soft_with_measurement": soft_bad,
            "soft_mentions": soft_ok, "fails": fails}
...
def check_anchors() -> dict:
    ...
    return {"status": "FAIL" if fails else "PASS", "rows": len(rows), "anchors_used": sum(uses.values()),
            "distinct_used": len(uses), "uncited_rows": uncited, "fails": fails}
