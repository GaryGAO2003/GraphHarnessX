...
160 papers screened, 18 close-read.
...
is given by the breeder's equation,
\[
  R \;=\; h^2 S ,
\]
...
the other side. Expected cumulative gain over $R$ rounds is consequently
non-positive \emph{regardless of variation quality} --- which is why better
...
\begin{enumerate}
  \item \textbf{The actuator demonstrably reached a lesion at least once.}
        ...
        family --- wrong-closure, 16 of the 44 swing tasks \F{31} --- which
...
that worked, and it does not explain the treadmill. Those two are
readout and inheritance failures, and they are what the proposition
addresses.
...
\paragraph{Leak-route contamination.} Twelve of the 44 swing tasks are
...
Noop-round batching (\$36.3 to \$11.8 per round \F{13}), single-shot
...
$1.6$ to $2.1$ times the bed rate \F{41}\F{44}\F{45}, rather than the whole
bed.
...
committing budget --- and the alternative is the readout itself, which is
