...
\begin{enumerate}
  \item \textbf{Headroom.} On BrowseComp, the published benchmark closest
        ...
        effort, $73.2$ at maximum --- which leaves an effect somewhere to
        appear.\footnote{DeepSeek-V4 model card, Hugging Face:
...
is a property of the task --- widens this to $[16.5\%, 23.9\%]$ \F{57}.
\emph{The noise floor is a property of the bed, not of a run.}
...
What \emph{pre-specified} rests on is stated here, because no external
registry was used. The cluster-transfer and canary designs, with their
...
endpoint is the pre-specified prospective batch and not any of them.
