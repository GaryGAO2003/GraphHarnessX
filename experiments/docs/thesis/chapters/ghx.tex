...
\begin{itemize}
  \item \textbf{Byte-pinned substrate.} The official system is vendored at
        ...
        suite. The baseline arm therefore runs the authors' system rather
        than a re-implementation of it; without this, every between-arm
        difference is confounded with maintenance.
  \item \textbf{Zero core edits.} No byte of the vendored tree is modified.
        Every GHX mechanism attaches at a seam, which makes the first
...
The released harness is not unstructured --- it is a configuration file
read by a runloop. Four things change when the same surface becomes a
typed graph, and they are worth separating because only the fourth is
specific to this thesis.

\begin{enumerate}
  \item \textbf{Order becomes decided rather than emergent.} In the
        release, what runs before what follows from list order and from
        registration side effects at import time. With $G$ as the single
        ordering authority the executed pipeline cannot disagree with the
        declared one, and a test says so.
  \item \textbf{An edit acquires an identity.} A genotype hash over $G$
        makes ``the same candidate'' a decidable question and gives the
        novelty ledger a key. Over free text it is not decidable at all:
        nothing downstream can confirm that a candidate document and the
        applied change are one intervention (\S\ref{sec:fielded}).
  \item \textbf{Illegality is caught at build time.} A
        write\,$\to$\,reload\,$\to$\,re-graph round trip and a
        hash-contract test refuse a malformed edit before a rollout pays
        for it. The measured consequence is that the share of candidates
        dying malformed falls from $30.5\%$ to $14.5\%$ \F{53}.
...
These rules cost coverage and buy admissibility: a gate that trusts a
...
did it behave the same way. GHX separates them with three nested hashes.
...
rules it is a sound over-approximation of the causal ancestry, not a
counterfactual, and is nowhere treated as one.
...
a reach of $86.0\%$ in the no-graph arm \F{4}. Every injection is recorded in a manifest, so what a role saw is
...
The objection to any evidence-injection result is that the treated arm was
given more to read. By construction, the rendered cone is a \emph{map, not a payload}: it
...
Together they support a narrower claim than ``the graph helped'':
structuring the evidence reduces its volume by an order of magnitude while
increasing the density and validity of its citations. Whether that improves
outcomes is a separate question, answered in the negative at this bed's
resolution in Chapter~\ref{ch:results}. Which component earns the citation
improvement is a third question \S\ref{sec:what-graph-did} leaves open, no
arm isolating the three candidate causes.
...
execution as anything but prose. Across the three graph seeds, eighteen
candidates cleared all five official gates and were then found to name a
node that never appeared in the execution graph of the failure they cited
\F{53}. A second reason is a trap in the official ladder itself: the
...
What changed is not the reach of the edit surface. Typed candidate spaces already exist in the literature
(\S\ref{sec:structured-spaces}), and the official loop could already edit
the processor plane. In the no-graph baseline arm it authored new processor \emph{classes} as source
...
\texttt{processor} was that campaign's largest candidate bucket. GHX's
contribution is therefore not the ability to add a node.
...
with zero collateral damage and zero conversions \F{11}\F{20}. It is an
aliveness proof --- the channel works end to end, from the loop's own
reasoning to a live runtime rule --- and not a result.
...
\begin{itemize}
  ...
  \item[\textbf{R1}] \emph{Did it ever execute?} --- answered by the replay
...
\subsection*{Offered as a standard}

Levels-of-evidence ladders do load-bearing work in other fields --- GRADE
in clinical medicine, SAE J3016 for driving automation --- by forcing a
system to state which rung its claims stand on.

Every self-evolving harness surveyed in \S\ref{sec:self-evolving} admits
candidates at \textbf{R0}: it checks that an edit landed, then selects on
aggregate score. None reports R1 or above. Given a recorded execution
graph, R1 and R2 are mechanical; R3 is where this thesis's evidence places
...
Two limitations follow from the design decisions of this chapter.
...
$549/549$ calls as used \F{8}. The false signal is verifiably gone, but the replacement carries no
information, so its presence cannot be read as a measurement.
Chapter~\ref{ch:results} repeats this where the removal result is
reported.
