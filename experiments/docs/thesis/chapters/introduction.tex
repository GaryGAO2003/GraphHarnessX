...
\emph{Why not just patch the twenty-character threshold, then?} Because
twenty was never the defect: the loop edits itself through free text, so
nothing downstream can confirm that a candidate document and the applied
change describe the same intervention, and four channels meant to carry
an edit's consequences back to the next edit are written but never
delivered (Chapter~\ref{ch:baseline}). A narrower number cannot repair an
identity problem or a broken feedback loop. GHX attaches to the whole
candidate-and-evidence surface for that reason, not to raise or lower one
...
\begin{description}[leftmargin=3.2em,style=nextline]
  ...
  \item[C4] A pre-specified test of H1 and its rejection on declared
...
\paragraph{How the rest of this document is organized.}
Chapter~\ref{ch:related} places this thesis against four literature
routes and concedes, table in hand, what the five nearest systems already
hold. Chapter~\ref{ch:baseline} is \emph{formative and diagnostic}: it
runs the official system at scale and establishes, by construction and by
measurement, that its evidence column is blind. Chapter~\ref{ch:ghx}
describes GraphHarnessX, the graph-native runtime built in response, and
what each of its gates checks. Chapter~\ref{ch:design} sets out the
measurement apparatus --- same-configuration re-measurement, resolution
laws, the pre-specified efficacy trial --- that
Chapter~\ref{ch:results} is read through. Chapter~\ref{ch:results}
\emph{carries the thesis's main results}: the verdict on H1, the readout
ladder's live rungs, and the sweeps that locate the missing organ.
Chapter~\ref{ch:discussion} states the readout proposition formally,
weighs it against the capability-wall rival account, and names the
experiment that would separate them. Chapter~\ref{ch:conclusion} restates
the claims against their evidence and closes with the durable artifacts.
Appendix~\ref{app:deviations} registers every deviation from the released
system and every scope ruling with its date; Appendix~\ref{app:ops}
narrates how the campaigns were actually run; Appendix~\ref{app:ledger}
is the fact ledger every number in this document anchors to.
