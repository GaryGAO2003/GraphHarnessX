...
(Chapter~\ref{ch:related}), which is why it was chosen as the object of
study.

\subsection*{The release against the paper}

The paper's GAIA headline is a $+13.6$-point gain that appears only once
variants are isolated inside an ensemble~\cite{aegis}. The variant pool and
ensemble routing it rests on are absent from the released code,
...
were flown before the graph-layer fixes of 19--26 August 2026, each
pinned by a regression test (Appendix~\ref{app:deviations}). They are
excluded from every number and every narrative in this thesis, and are
named here only so that the count of campaigns flown is complete. Two executed rounds
...
and causal ancestor cones over it (over-approximations of what could have
shaped a failure, never counterfactuals). Four switches are stacked cumulatively
...
\paragraph{Timing of the three scope rulings.}
\begin{enumerate}
...
\emph{No ruling was made after seeing the number it would change in the
favourable direction}: ruling 1 lowered the graph arm's pooled localization
lift once the shakedown campaigns were excluded \F{9b}, the checkable case
(per-ruling breakdown: Appendix~\ref{app:deviations}).
