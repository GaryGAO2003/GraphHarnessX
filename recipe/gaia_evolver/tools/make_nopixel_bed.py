...
parser.add_argument("--input", type=Path, default=Path(__file__).resolve().parents[1] / "data/webthinker_gaia_dev.json")
parser.add_argument("--output", type=Path)
parser.add_argument("--manifest", type=Path)
args = parser.parse_args()
SRC = args.input.resolve()
DST = (args.output or SRC.with_name("webthinker_gaia_dev_nopixel.json")).resolve()
MANIFEST = (args.manifest or DST.with_suffix(".README.md")).resolve()
...
DROP = {
    "7a4a336d-dcfa-45a0-b014-824c7619e8de": "the racetrack shown at the two-minute mark of a Let's Play video — the answer is a "
    "video frame; no transcript, description or wiki states which track is on screen then",
    "624cbf11-6a41-4692-af9c-36b3e5ca3130": "the rhyme on a headstone visible in the BACKGROUND of a photo — the answer is a "
    "region of one image, never transcribed anywhere",
    "0e9e85b8-52b9-4de4-b402-5f635ab9631f": "'the latest year date WRITTEN IN THE IMAGE on the webpage' — the question names the "
...
KEPT_BORDERLINE = {
    "5f982798-16b9-4051-ab57-cfc7ebdb2a91": "asks for a time span shown in an arXiv figure, but arXiv papers state their measured "
    "spans in the caption and body text — a reading task, not a perception task",
    "d5141ca5-e7a0-469f-bf3e-e773507c86e2": "asks WHEN a picture was added to a Wikipedia page — answered by revision history, "
    "which is text and has an API",
    "0bdb7c40-671d-4ad1-9ce3-986b159c0ddc": "NASA Astronomy Picture of the Day — the explanation text under every APOD names what "
    "is in the frame; passes 9/12 today",
}

if not SRC.is_file():
    parser.error(f"missing input dataset: {SRC}")
...
if len(dropped) != len(DROP):
    parser.error(f"expected {len(DROP)} drops, matched {len(dropped)}")
